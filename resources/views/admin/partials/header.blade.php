    ...
    <div class="flex items-center justify-between px-4 lg:px-6 py-4">
        ...
        <div class="flex items-center space-x-4">
            ...
            <button @click="sidebarOpen = true"
                class="lg:hidden p-2 text-blue-600 hover:text-blue-700 hover:bg-blue-50 rounded-xl transition-all duration-200">
                <i class="fas fa-bars text-lg"></i>
            </button>

            <div class="flex flex-col">
                <h1 class="text-xl lg:text-2xl font-bold text-blue-900 tracking-tight flex items-center flex-wrap gap-2">
        ...
        <div class="flex items-center space-x-3 lg:space-x-4">
            ...
            <div class="relative group" x-data="{ open: false }">
                <button @click="open = !open"
                    class="flex items-center space-x-2 lg:space-x-3 p-2 rounded-xl hover:bg-blue-50 transition-all duration-200">
                    <!-- Avatar -->
                    <div class="relative">
                        <div
                    ...
                    <div class="hidden lg:block text-left">
                        <p class="text-sm font-medium text-blue-900 leading-tight">{{ auth()->user()->name }}</p>
                        <p class="text-xs text-blue-600 capitalize">{{ auth()->user()->role }}</p>
                    </div>

                    <i class="fas fa-chevron-down text-blue-400 text-xs transition-transform duration-200"
                ...
                <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-200"
                    ...
                    <div class="px-3 py-2 border-b border-blue-100">
                        <p class="text-sm font-semibold text-blue-900 truncate">{{ auth()->user()->name }}</p>
                        <div class="flex items-center justify-between mt-1">
                            <span class="text-xs text-blue-600 truncate">{{ auth()->user()->email }}</span>
                            <span
                    ...
                    <div class="py-1">
                        <a href="{{ route('bookmart') }}" target="_blank"
                            class="flex items-center px-3 py-2 text-sm text-blue-700 hover:bg-blue-50 hover:text-blue-600 transition-colors duration-200 group">
                            <i
                                class="fas fa-external-link-alt text-blue-400 group-hover:text-blue-500 mr-2.5 w-4 text-center text-xs"></i>
                            Ir al Sitio Web
                        </a>

                        <div class="my-1 border-t border-blue-100"></div>

                        <a href="#"
                            class="flex items-center px-3 py-2 text-sm text-blue-700 hover:bg-blue-50 hover:text-blue-600 transition-colors duration-200 group">
                            <i
                                class="fas fa-cog text-blue-400 group-hover:text-blue-500 mr-2.5 w-4 text-center text-xs"></i>
                            Preferencias
                        </a>
                    </div>

                    <!-- Footer del Dropdown -->
                    <div class="px-3 py-1 border-t border-blue-100">
